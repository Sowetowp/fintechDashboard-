    ...
    switch (action.type) {
        case "open":
            return Object.keys(state).reduce((acc, cuu) => {
                acc[cuu] = cuu === action.val.toString()
                return acc;
            }, {})
        case "close":
            return Object.keys(state).reduce((acc, cuu) => {
            ...
            <main className='w-full bg-white dark:bg-blue-950'>
                <section className='max-w-7xl w-full mx-auto'>
                    <div className='w-[22%] py-3'>
                        <div onClick={()=> dispatch({type: "open", val: "dashboard"})} className='flex justify-around flex-wrap items-center group'>
                            ...
                            <div className={`w-full py-2 hidden ${drop.dashboard ? "block" : "hidden"}`}>
